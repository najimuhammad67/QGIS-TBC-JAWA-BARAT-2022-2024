...
map
  .getView()
  .fit(
    [12301926.760061, -779927.300601, 12381536.871896, -737278.342418],
    map.getSize()
  );
          ...
          for (var n = 0; n < clusteredFeatures.length; n++) {
            ...
            for (var i = 0; i < currentFeatureKeys.length; i++) {
              if (currentFeatureKeys[i] != "geometry") {
                ...
                var labelType = fieldLabels[key];
                var alias = fieldAliases[key] || key;
                var imageType = fieldImages[key];
                if (labelType == "inline label") {
                  popupField += "<th>" + alias + ":</th><td>";
                } else {
                  popupField += '<td colspan="2">';
                }
                if (labelType == "header label") {
                  popupField += "<strong>" + alias + ":</strong><br />";
                }
                if (imageType != "Photo") {
                  popupField +=
                    clusterFeature.get(key) != null
                      ? Autolinker.link(String(clusterFeature.get(key))) +
                        "</td>"
                      : "";
                } else {
                  popupField +=
              ...
              if (imageType != "Photo") {
                popupField +=
                  currentFeature.get(key) != null
                    ? Autolinker.link(String(currentFeature.get(key))) + "</td>"
                    : "";
              } else {
                popupField +=
                  currentFeature.get(key) != null
            ...
            for (var n = 0; n < clusteredFeatures.length; n++) {
              ...
              for (var i = 0; i < currentFeatureKeys.length; i++) {
                if (currentFeatureKeys[i] != "geometry") {
                  ...
                  if (
                    layer.get("fieldImages")[currentFeatureKeys[i]] != "Photo"
                  ) {
                    popupField +=
                      clusterFeature.get(currentFeatureKeys[i]) != null
                        ? Autolinker.link(
                            String(clusterFeature.get(currentFeatureKeys[i]))
                          ) + "</td>"
                        : "";
                  } else {
                    popupField +=
